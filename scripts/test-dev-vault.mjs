...
assert(vault.includes('data-dv-tab="status"'), 'hub has Status tab');
assert(vault.includes('devStatusBoard'), 'hub uses Status Board');
assert(vault.includes("data-dv-tab=\"dashboard\""), 'hub has Dashboard');
assert(vault.includes("data-dv-tab=\"reports\""), 'hub has Raporty');
assert(vault.includes("data-dv-tab=\"tools\""), 'hub has Narzędzia');
assert(vault.includes("data-dv-tab=\"intelligence\""), 'hub has Inteligencja');
assert(vault.includes("data-dv-tab=\"maintenance\""), 'hub has Utrzymanie');
assert(vault.includes("data-dv-tab=\"info\""), 'hub has Informacje');
assert(vault.includes('loadControlCenterMetrics'), 'loads scores from existing reports');
...
assert(vault.includes('openHealthDevPanel'), 'keeps Health open');
assert(vault.includes('openDeveloperDashboard'), 'keeps Dev dashboard open');
...
assert(vault.includes('Zarządzanie raportami'), '34C report manager section');
assert(vault.includes('reportManagerClient'), '34C client import');
assert(vault.includes('Usuń stare raporty'), '34C cleanup old reports');
