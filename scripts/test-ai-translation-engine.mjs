...
assert(/CACHE_TTL_MS|getCacheKey/.test(eng), 'cache TTL + composite key');
assert(/QUEUE_CONCURRENCY\s*=\s*1/.test(eng), 'serial API queue');
assert(/REQUEST_GAP_MS\s*=\s*750/.test(eng), 'request gap 750ms');
assert(/pendingByKey/.test(eng), 'session promise dedup');
assert(/isProviderAvailable/.test(eng), 'provider availability guard');
assert(/PROVIDER_RATE_LIMIT_MS\s*=\s*60_000/.test(eng), '429 cooldown 60s');
assert(/IS_LOCALHOST/.test(eng), 'localhost-only diagnostics');
...
assert(/libre not configured|isProviderAvailable|no-api-key/.test(eng), 'skip libre without api key');
